...
// Sampling
bool sampling = <<ENABLE_SAMPLING>>;  // You need to disable sampling to perform verification.
...
import "./<<EXT_NAME>>" {
    ...
    void extPushTopology(phase_t i, node_t& data[NUM_PORTS]);
...
packet_t maxPacketsBuffered() {
    packet_t max = 0;
    for (node : node_t) {
        packet_t buffered = packetsAtNode(node);
        if (buffered > max) {
            max = buffered;
        }
    }
